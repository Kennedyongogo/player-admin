...
// Simple county normalization (can be enhanced later)
const RAW_COUNTIES = [
  "Baringo",
  "Bomet",
  "Bungoma",
...
const normalizeCountyName = (input) => {
  if (!input || typeof input !== "string") return null;
  const cleaned = input.trim();
  if (!cleaned) return null;
  const lower = cleaned.toLowerCase();
  for (const county of RAW_COUNTIES) {
    if (county.toLowerCase() === lower) return county;
  }
  return null;
};
